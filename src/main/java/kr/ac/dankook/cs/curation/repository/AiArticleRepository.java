...
/**
 * RecommendedArticle 엔티티용 JPA 리포지토리
 * - 추천 뉴스 기사에 대한 CRUD 및 검색 기능 제공
 */
@Repository
public interface AiArticleRepository extends JpaRepository<AiArticle, Long> {

    /**
     * 특정 URL을 가진 추천 뉴스가 이미 존재하는지 확인
     * - 중복 저장 방지에 사용
     * @param url 뉴스 URL
    ...
    /**
     * 제목에 특정 키워드가 포함된 추천 뉴스 검색
     * - 대소문자 무시
     * @param keyword 검색 키워드
     * @return 매칭된 기사 목록
     */
    List<AiArticle> findByTitleContainingIgnoreCase(String keyword);
    ...
    List<AiArticle> findByPublishedAtBefore(LocalDateTime date);
}
